...
function generateId() {
  return crypto
    .randomBytes(6) // 6 bytes ~ 12 caractères hex
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      if (results.length === 0) {
        return res
          .status(400)
          .json({ message: "Numéro de compte n'existe pas" });
      }
...
app.post("/profil", (req, res) => {
  ...
  if (!idUsers || isNaN(idUsers)) {
    return res
      .status(400)
      .json({ error: "L'idUsers est requis et doit être un nombre valide." });
  }
...
app.post("/userExiste", (req, res) => {
  ...
  db.query("select * from users where idUser = ?", [iduser], (err, results) => {
    if (err) return res.status(500).json({ message: "huhuh" });
    if (results.length === 0) return res.status(401).json({ user: false });
...
app.post("/solde", (req, res) => {
  ...
  if (!idUser || isNaN(idUser)) {
    return res
      .status(400)
      .json({ error: "L'idUser est requis et doit être un nombre valide." });
  }
  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      ...
      (err, results) => {
        ...
        if (results.length === 0)
          return db.rollback(() =>
            res.status(404).json({ error: "Compte source introuvable" })
          );
        if (results[0].solde < montantDebiter)
          return db.rollback(() =>
...
app.post("/retrait", (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      "SELECT idCompte, solde FROM comptes WHERE numeroCompte = ?",
      [numeroCompteClient],
      (err, results) => {
        if (err)
          return db.rollback(() =>
        ...
        if (soldeClient < montant)
          return db.rollback(() =>
            res.status(400).json({ error: "Solde client insuffisant" })
          );

        // Débiter le compte client
        db.query(
          "UPDATE comptes SET solde = solde - ? WHERE idCompte = ?",
          [montant, idCompteClient],
          (err) => {
            if (err)
              return db.rollback(() =>
                res.status(500).json({ error: err.message })
            ...
            db.query(
              "UPDATE comptes SET solde = solde + ? WHERE idCompte = ?",
              [montantTotal, idCompteDistributeur],
              (err) => {
                if (err)
                  return db.rollback(() =>
                    res.status(500).json({ error: err.message })
...
app.get("/comptesByrole/:role", (req, res) => {
  ...
  if (!role || typeof role !== "string" || role.trim() === "") {
    return res
      .status(400)
      .json({ error: "Le rôle est requis et doit être une chaîne valide." });
  }
  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ error: "Aucun compte trouvé pour ce rôle." });
      }
...
app.post("/transfert", (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      error: "idCompte, compteDestinataire et montant (positif) sont requis.",
    });
  }
  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      "SELECT solde, idusers FROM comptes WHERE idCompte = ?",
      [idCompte],
      (err, results) => {
        if (err) {
          return db.rollback(() => {
        ...
        db.query(
          ...
          (err, results) => {
            if (err) {
              return db.rollback(() => {
                console.error(
                  "Erreur lors de la vérification du compte destinataire:",
                  err
                );
            ...
            if (results.length === 0) {
              return db.rollback(() => {
                res
                  .status(404)
                  .json({ error: "Compte destinataire non trouvé." });
              });
            }
              ...
              (err, result) => {
                if (err) {
                  return db.rollback(() => {
                    ...
                    res
                      .status(500)
                      .json({ error: "Erreur interne du serveur4." });
                  });
                }
                  ...
                  (err, result) => {
                    if (err) {
                      return db.rollback(() => {
                        ...
                        res
                          .status(500)
                          .json({ error: "Erreur interne du serveur5." });
                      });
                    }
                      ...
                      (err, result) => {
                        if (err) {
                          return db.rollback(() => {
                            ...
                            res
                              .status(500)
                              .json({ error: "Erreur interne du serveur6." });
                          });
                        }
                        ...
                        db.commit((err) => {
                          if (err) {
                            return db.rollback(() => {
                              ...
                              res
                                .status(500)
                                .json({ error: "Erreur interne du serveur7." });
                            });
                          }
                          ...
                          res.status(200).json({
                            ...
                            nouveauSoldeSource:
                              compteSource.solde - montantTotal,
                            montantTransfere: montant,
                            frais: frais,
                          });
                        });
...
app.get("/nbrClient", (req, res) => {
  db.query("select count(id) as nbrClient from clients", (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
  ...
  db.beginTransaction((err) => {
    if (err) {
      return res.status(500).json({
        error: "Erreur de début de transaction",
        details: err.message,
      });
    }
      ...
      (err, result) => {
        ...
        if (result.length === 0) {
          return db.rollback(() => {
            res
              .status(404)
              .json({ message: "Transaction introuvable", status: 404 });
          });
        }
        ...
        const {
          ...
          montant,
          frais,
          idCompteSource: source,
          idCompteDestinataire: dest,
        } = result[0];
        ...
        if (type === "transfert") {
          ...
          return db.rollback(() => {
            res
              .status(400)
              .json({ message: "Type de transaction non supporté" });
          });
        }
